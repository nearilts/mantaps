...
const ListTabTerpakai = ({ navigation }) => {
    ...
    const fetchdatas = async (search = '') => {
        try {
            ...
            const response = await axios.get(`${url}get-terpakai?name=${search}`, {
                headers: {
    ...
    const renderItem = ({ item }) => (
        <View>
            <TouchableOpacity 
                onPress={() => navigation.navigate("FormKembaliScreen", item)}
            >
...
export default ListTabTerpakai;
