...
const ListTabScreen = ({ navigation }) => {
    ...
    const btnTersedia = () => {
        const tersedia = originalDatas.filter(item => item.status === 'Tersedia');
        setDatas(tersedia);
    };

    const btnTerpakai = () => {
        const terpakai = originalDatas.filter(item => item.status === 'Terpakai');
        setDatas(terpakai);
    };
    ...
    const renderItem = ({ item }) => (
        <View>
            <TouchableOpacity 
            // onPress={() => navigation.navigate("ListKapalScreen")}
            >
        ...
        <SafeAreaView style={{ backgroundColor: COLORS.white, flex: 1 }}>
            <View style={{ top: 60 }}>
                ...
                <View style={{ paddingTop: 20, flexDirection: 'row', justifyContent: 'space-between', marginLeft: 20, marginRight: 20 }}>
                    <TouchableOpacity onPress={btnTersedia} style={{ backgroundColor: COLORS.green, width: 180, height: 45, borderRadius: 15, justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ color: COLORS.white, fontSize: 16 }}>Tersedia</Text>
                    </TouchableOpacity>

                    <TouchableOpacity onPress={btnTerpakai} style={{ backgroundColor: COLORS.red, width: 180, height: 45, borderRadius: 15, justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ color: COLORS.white, fontSize: 16 }}>Terpakai</Text>
                    </TouchableOpacity>
                </View>
